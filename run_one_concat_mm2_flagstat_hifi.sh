...
OUT_FLAGSTAT_DIR="${OUT_FLAGSTAT_DIR:-/lustre10/home/raulnmateos/Japanese_Pangenome/Pipeline/Figure_2/required_data/Flagstat_HiFi}"

# Use node-local scratch when possible (best), else fall back to a Lustre tmp dir.
if [[ -n "${SLURM_TMPDIR:-}" ]]; then
  WORKDIR="${WORKDIR:-$SLURM_TMPDIR/hifi_mm2_work}"
else
  WORKDIR="${WORKDIR:-/lustre10/home/raulnmateos/Japanese_Pangenome/Pipeline/Figure_2/required_data/tmp_hifi_mm2_work}"
fi

# One thread count for all steps
THREADS="${THREADS:-${SLURM_CPUS_PER_TASK:-16}}"

mkdir -p "$OUT_FLAGSTAT_DIR" "$WORKDIR"
...
run_one() {
  ...
  echo "INFO: [$ID][$hap] minimap2/samtools threads=$THREADS"
  minimap2 --cs -L -Y -t "$THREADS" -ax map-hifi "$fasta" "$FASTQ" \
    | samtools sort -@ "$THREADS" -o "$tmpbam" -

  mv -f "$tmpbam" "$bam"
  samtools index -@ "$THREADS" "$bam"
  samtools flagstat -@ "$THREADS" "$bam" > "$flagout"

  # Remove BAM and BAI after flagstat
  rm -f "$bam" "${bam}.bai"
}
...
echo "DONE: $ID (flagstats written; BAM and temp FASTQ cleaned)"
